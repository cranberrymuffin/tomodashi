...
class AnimatedAdultNode: SKSpriteNode {
    private let originalSize: CGSize
    private var sceneSize: CGSize = CGSize.zero
    
    init(size: CGSize) {
        self.originalSize = size
        let texture = SKTexture(imageNamed: "adult")
    ...
    private func setupNode() {
        print("AnimatedAdultNode: Setting up adult node")
    ...
    private func positionOnGround() {
        ...
        // Position adult on top of the ground
        let groundY = 12.5 + self.size.height/2
        self.position = CGPoint(x: 0, y: groundY)
        print("AnimatedAdultNode: Positioned at \(self.position)")
    }
    
    func startAnimation() {
        print("AnimatedAdultNode: Starting animation (no animation implemented yet)")
        // TODO: Implement adult animations
    }
    
    func stopAnimation() {
        print("AnimatedAdultNode: Stopping animation")
        removeAllActions()
    }
    
    func addJoyfulWiggle() {
        print("AnimatedAdultNode: Adding joyful wiggle")
